...
class CreateAppointmentService {
    ...
    public async execute({
        ...
        if (getHours(appointmentDate) < 8 || getHours(appointmentDate) > 17) {
            throw new AppError("You can only create appointment netween 8am am 5pm");
        }

        const findAppointmentInSendDate = await this.appointmentsRepository.findByDate(appointmentDate);
